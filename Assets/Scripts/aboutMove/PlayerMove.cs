...
{
    Vector2 playerPos;//플레이어 위치를 일일이 써주면 더러워져서 지정
    ...
    void Start()
    {
        Maker = this.GetComponent<blockMakerTwo>();
        ...
        if (nowX < 5)//플레이어가 6번째 블럭에 서있으면 오른쪽으로 갈 수 없으므로
        {
            ...
            for (int i = 0; i < Maker.Block.Count; i++)
            {
                if (Maker.Block[i][nowX] != null)
                {
                    ...
                    difX = 1;
                    difY = (int)Maker.Block[i][nowX].gameObject.transform.position.y - (int)(playerPos.y - 1.515f);//무조건 소수점에서 내림이 되기 때문에
                    if (difY <= 0)
                    {
                        this.gameObject.GetComponent<helpMoveYdown>().enabled = true;
                    }
                    else if (difY > 0)
                    {
                        this.gameObject.GetComponent<helpMoveYup>().enabled = true;
                    }
                    right.SetActive(!right.active);
                    left.SetActive(!left.active);
                    down.SetActive(!down.active);
                    break;
                }
            }
        }
        ...
        if (nowX > 0)//플레이어가 1번째 블럭에 서있으면 오른쪽으로 갈 수 없으므로
        {
            ...
            for (int i = 0; i < Maker.Block.Count; i++)
            {
                if (Maker.Block[i][nowX] != null)
                {
                    Destination = Maker.Block[i][nowX].gameObject.transform.position;//가장 높은 곳이 가야할 곳이므로 목적지로 지정한다.
                    difX = -1;
                    difY = (int)Maker.Block[i][nowX].gameObject.transform.position.y - (int)(playerPos.y - 1.515f);
                    if (difY <= 0)
                    {
                        this.gameObject.GetComponent<helpMoveYdown>().enabled = true;
                    }
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        if (collision.gameObject.tag == "Enemy"&&DownAttack==false)
        {
            Debug.Log("Gemaasodosa");
            this.gameObject.GetComponent<helpMoveYup>().enabled = false;
            this.gameObject.GetComponent<helpMoveYdown>().enabled = false;
            Application.LoadLevel("GameOverScene");
        }
        else if(collision.gameObject.tag == "Enemy" && DownAttack == true)
        {
